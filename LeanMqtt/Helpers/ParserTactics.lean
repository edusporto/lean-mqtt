...
/--
  `step_option h → v step_proof` is a tactic macro designed to unroll
  a single step of a `bind` operation over an `Option` monad.

  It applies `Option.bind_run_success` to crack the `bind` step `h`, extracting:
  - `v`: the parsed value from this step
  - `step_proof`: the proof that this specific step succeeded
-/
macro "step_option" h:ident "→" v:ident step_proof:ident : tactic =>
  `(tactic| (
    replace $h := Option.bind_run_success $h
    obtain ⟨$v:ident, $step_proof:ident, $h:ident⟩ := $h
  ))
